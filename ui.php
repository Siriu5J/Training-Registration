<?php
add_shortcode('staff_form','staffFormCreation');
add_shortcode('view_staff', 'viewEditStaff');
add_shortcode('register_training', 'eventRegistration');

/*
STAFF PROFILE REGISTRATION FORM
 */
function staffFormCreation() {
        ...
        if ($event != '' && $staff != '') {
            ...
            if ($training->open_time < $time_now && $training->close_time > $time_now && ($training->limit_max == 0 || $training->max == -999 || $training->max - $training->num_reg > 0)) {
                ...
                if ($wpdb->get_var("SELECT COUNT(*) FROM $reg_table WHERE `staff` = $staff AND `event_id` = $event") == 0) {
                    ...
                    ?>
                    <div style="background-color: #5ac18e; color: white; padding: 15pt; border-radius: 5px; position: initial; left: 50%;"><?php echo idtoName($staff) ?> is successfully registered to <?php echo $training->event_name.' at '.$training->location ?></div>
                    <?php
                } else {
                    ?>
                    <div style="background-color: #ff4040; color: white; padding: 15pt; border-radius: 5px; position: initial; left: 50%;"><?php echo idtoName($staff) ?> has already been registered to <?php echo $training->event_name.' at '.$training->location ?>. </div>
                    <?php
                }
            } else {
                ?>
        ...
        <div>
            ...
            <table>
                ...
                foreach ($trainings_to_show as $training) {
                    ?>
                    <tr>
                        <th colspan="3"><?php echo $training->event_name.' ('.availability($training).')' ?></th>
                    ...
                    if ($show_available == 1) {
                        ...
                        <tr>
                            ...
                            <td colspan="3"><?php echo spotsOpen($training->max, $training->num_reg); ?></td>
                        </tr>
                        <?php
                    }
                    ?>
        ...
        <form id="reg-event" name="reg-event" method="post" action="<?php echo $_SERVER['REQUEST_URI'];?>">
            ...
            <select id="staff" name="staff">
                ...
                foreach ($staff_available as $staff) {
                    echo '<option value="'.$staff->id.'">'.idtoName($staff->id).'</option>';
                }
                ?>
            </select>
            <br>
...
function viewEditStaff() {
            ...
            <form id="select-staff" name="select-staff" method="post" action="<?php echo $_SERVER['REQUEST_URI'];?>">
                ...
                foreach ($all_staff as $staff) {
                    ...
                    <tr>
                        ...
                        <td><?php echo idtoName($staff->id); ?></td>
    ...
    if ($_POST['edit-reg']) {
        ...
        <hr>
        <?php
        if (hasRemovables($staff_id)) {
            ?>
            <h4>Cancel Registrations for <?php echo idtoName($staff_id) ?>:</h4>
            ...
            ?>
            <h4>No registrations available for cancelling for <?php echo idtoName($staff_id) ?></h4>
            <?php
        }
